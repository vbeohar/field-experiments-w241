...
summary_table #this is the covariate imbalance check
...
cvcov1 <- vcovCL(m1,  cluster = d[, school_id]) # from multiwayvcov
cvcov2 <- vcovCL(m2,  cluster = d[, school_id])
cvcov3 <- vcovCL(m3,  cluster = d[, school_id])
...
# TO ASK ALEX: clustering is at the school level. cluster std errors wouldnt matter if the treatment assignement were at the classroom level, then things would be balanced at the school. 
# but the clustering only matters when treatment assignment is correlated with the cluser
