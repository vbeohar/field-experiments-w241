...
# vb code check
d_experiment[ , .(group_mean = mean(outcomes)), keyby = .(randomize(20))]
# d_experiment[ , .(group_mean = mean(outcomes)), keyby = .(randomize(20))][ , diff(group_mean)]


DT = data.table(x=rep(c("b","a","c"),each=4), y=c(1,3,6,8), v=1:12)
DT
DT[, sum(v), keyby=.(x)]
DT[, sum(y), keyby=.(randomize(6))]
...
sum(dist_sharp_null < ate) #check how many of randomized inferred values from the sharp null distribution are smaller than the ate

mean(dist_sharp_null < ate) #check how many of % randomized inferred values from the sharp null distribution are smaller than the ate
