...
#ate <- d[ , .(group_mean = _mean(Y)), keyby = .(Z)]

ate <- d[ , .(group_mean = mean(Y)), keyby = .(Z)][ , .('ate' = diff(group_mean))]
ate
...
4. Repeatedly sample the treatment assignment vector and under each assignment, compute the test statistic in the same manner as in step 1. 

```{r}

res <- NA
for (ri_loop in 1:1000){
  res[ri_loop] <- d[ , .(group_mean = mean(Y)), keyby = .(sample(Z))][ , .('ate' = diff(group_mean))]
...
hist(res[1000], col = 'black', breaks = 30)


mean(res >= 70)

mean(res >= 70) + mean(res <= 70)
```
